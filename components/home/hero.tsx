...
export function Hero() {
  ...
  return (
    <section
      ...
      className={`bg-cream p-3 md:p-4 ${open ? "relative z-40" : ""}`}
    >
      <div className="hero-flip-scene">
        <div
          className={`hero-flip-card relative min-h-[calc(100svh-1.5rem)] md:min-h-[calc(100svh-2rem)] ${
            open ? "hero-flip-card--open" : ""
          }`}
        >
          {/* Front — hero */}
          <div
            className={`hero-flip-face relative flex min-h-[calc(100svh-1.5rem)] items-center justify-center overflow-hidden rounded-[1.5rem] md:min-h-[calc(100svh-2rem)] md:rounded-[2rem] ${
          ...
          >
            ...
            <div className="relative z-10 mx-auto flex w-full max-w-3xl flex-col items-center px-5 py-24 text-center md:px-8 md:py-28">
              <h1 className="animate-fade-up max-w-3xl font-display text-4xl font-bold leading-[1.15] tracking-tight text-cream drop-shadow-[0_2px_18px_rgba(27,48,34,0.35)] sm:text-5xl md:text-6xl lg:text-[3.5rem]">
                {dict.hero.title}
              </h1>
          ...
          {/* Back — mobile menu in the same rounded hero shape */}
          <div
            className="hero-flip-face hero-flip-face--back absolute inset-0 z-10 overflow-hidden rounded-[1.5rem] md:rounded-[2rem] lg:hidden"
            aria-hidden={!open}
          >
            <MobileMenuFace isHome />
          </div>
        </div>
      </div>
    </section>
